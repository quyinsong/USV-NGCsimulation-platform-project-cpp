...
Panel::Panel(QWidget *obj,QString title,QString units,
             int maxValue,int minValue,int scaleMajor,int scaleMinor)
{
...
// 绘制刻度线
void Panel::drawScale(QPainter *painter) //绘制刻度线
{
    painter->save();
...
// 显示的单位，与数值
void Panel::drawNumericValue(QPainter *painter)
{
...
void Panel::widgetPaint()
{
    int width=m_obj->width();
    int height=m_obj->height();

    QPainter mpainter(m_obj);

    mpainter.setRenderHint(QPainter::Antialiasing);  /* 使用反锯齿(如果可用) */
    mpainter.translate(width/2, height/2);   /* 坐标变换为窗体中心 */
    int side = qMin(width, height);
    mpainter.scale(side / 200.0, side / 200.0);      /* 比例缩放 */

   drawCrown(&mpainter);                                 /* 画表盘边框 */
   drawScaleNum(&mpainter);                          /* 画刻度数值值 */
   drawScale(&mpainter);                                 /* 画刻度线 */
   drawTitle(&mpainter);                                 /* 画单位 */
   drawNumericValue(&mpainter);                      /* 画数字显示 */
   drawIndicator(&mpainter);                             /* 画表针 */
}  // fun subPaint
